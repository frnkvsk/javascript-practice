...
async function buttonHandler() {
    ...
    let all = await getPokemon(ALL_URL);
        
    randomNumbers.map(async e => {
        ...
        let {name, url} = all.data.results[e];
        title.innerText = name;

        let pokemon = await getPokemon(url);
        let speciesURL = pokemon.data.species.url;
        let imageURL = pokemon.data.sprites.front_shiny;
        
        let species = await getPokemon(speciesURL);
        description.innerText = species.data.flavor_text_entries
                                .filter(e => e.language.name == "en")[0].flavor_text;
        let img = new Image();
